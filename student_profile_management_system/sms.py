...
def search_data():
    query='SELECT * FROM student WHERE id=%s or name=%s or email=%s or mobile=%s or address=%s or gender=%s'
    mycursor.execute(query,(idEntry.get(),nameEntry.get(),emailEntry.get(),phoneEntry.get(),addressEntry.get(),genderEntry.get()))
    studentTable.delete(*studentTable.get_children())
    fetched_data=mycursor.fetchall()
    for data in fetched_data:
        studentTable.insert('',END,values=data)
...
def add_data():
     if (idEntry.get() == '' or nameEntry.get() == '' or phoneEntry.get() == '' or 
        emailEntry.get() == '' or addressEntry.get() == '' or genderEntry.get() == '' ):
        messagebox.showerror('Error', 'All Fields are required', parent=screen)
        return
    # convert name to uppercase
     name=nameEntry.get().strip().upper()
     
    # check if name contain only letters
     if not name.replace(" ","").isalpha():
         messagebox.showerror('Error','Name must contain only alphabets and be in uppercase.',parent=screen)
         return 
    # validate Gender(Only "Male" or "Female")
     gender = genderEntry.get().strip().capitalize()
     if gender not in ["Male", "Female"]:
         messagebox.showerror('Error','Gender must be  either "male" or "Female".',parent=screen)
         return
         

     try:
        # Check if the ID already exists
        id_value=int(idEntry.get()) # convert to integer
        check_query = 'SELECT * FROM student WHERE id = %s'
        mycursor.execute(check_query, (id_value))
        ...
        check_mobile_query='SELECT * FROM student WHERE mobile = %s'
        mycursor.execute(check_mobile_query,(phoneEntry.get()))
        existing_mobile=mycursor.fetchone()
        
        if existing_mobile:
            messagebox.showerror('Error','Mobile Number already exists! Please use a unique number. ',parent=screen)
            return

        # If ID is unique, insert the new student
        query = 'INSERT INTO student (id, name, mobile, email, address, gender) VALUES (%s, %s, %s, %s, %s, %s)'
        mycursor.execute(query, (
...
def connect_database():
    def connect():
        ...
        try:
            ...
            'address varchar(100),gender varchar(20))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
...
logo_image=PhotoImage(file='student_profile_management_system/student.png')
